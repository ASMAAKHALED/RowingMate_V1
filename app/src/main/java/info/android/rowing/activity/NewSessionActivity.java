...
public class NewSessionActivity extends AppCompatActivity implements LocationListener, SensorEventListener, IBaseGpsListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DatabaseReference usersRef = ref.child("strokeRate");
        final DatabaseReference usersRef2 = ref.child("speed");


        //final RowerData mRowerData=new RowerData( strRate,  nCurrentSpeed,  angle) ;

        //usersRef.setValue(mRowerData);
        usersRef.setValue(strRate);
        usersRef2.setValue(nCurrentSpeed);
...
*/
        ...
        new Timer().scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                // MessageTx friendlyMessage = new MessageTx(x, mUsername);
                // mMessageDatabaseReference.push().setValue(x);
...
//what ever we want to send we can make a child  for it
                // usersRef.setValue(x);
               // usersRef.push().setValue(mRowerData);
                usersRef.push().setValue(strRate);
                usersRef2.push().setValue(nCurrentSpeed);



            }
        }, 0, 7000); // 5 sec just for try
        ...
        mAuthStateListener=new FirebaseAuth.AuthStateListener() {
            @Override
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) {//this method is called in on resume
                FirebaseUser user=firebaseAuth.getCurrentUser();
                if(user!=null){
                    //user is signed in
                    onSignedInInitialized(user.getDisplayName());
                    // Toast.makeText(MainActivity.this,"you are here since" ,Toast.LENGTH_SHORT).show();
